...
$(175/39)^{0.75} = 3.1\times$ to within 3\%,
using no free parameters beyond the fitted $\alpha=0.25$.
...
Section~\ref{sec:methods} describes the power-law model, OLS
estimator, bootstrap confidence intervals, and population-level
tests.
Section~\ref{sec:data} details the sample construction, fund
classifications, and data sources.
Section~\ref{sec:results} presents the main empirical findings,
including fund-level scaling estimates, the quant--pod separation,
mechanism interpretation, and capital efficiency analysis.
Section~\ref{sec:robust} examines robustness to alternative
classifications, non-monotone trajectories, panel regression
specifications, and a Bayesian hierarchical model.
Section~\ref{sec:oos} reports out-of-sample validation on 26
held-out funds across four strategy types.
Section~\ref{sec:survivorship} analyses survivorship bias and its
effect on the estimated $\alpha$ distribution using an expanded
40-fund universe.
Section~\ref{sec:discussion} discusses what the evidence does and
does not establish, including causality, the automation
interpretation, and connections to scaling laws in other domains.
...
For Man~Group ($\alpha = 0.25$, audited), the prediction is
$(175/39)^{0.75} \approx 3.1\times$ efficiency gain over the
2010--2024 AUM expansion.
